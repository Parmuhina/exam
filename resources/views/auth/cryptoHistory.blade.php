<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Crypto transactions history') }}
        </h2>
    </x-slot>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 ">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg ">
                <div class="p-6 text-gray-900">
                    ...
                    <form method="POST" action="{{ route('cryptoFilter') }}">
                        @csrf
                                ...
                                <select class="form-select" name="accountNumber" id="accountNumber">
                                    @foreach ($accounts as $account)
                                        @if (strtoupper(substr($account->account_number, 0, 1)) == 'C')
                                            <option
                                                value="{{$account->account_number}}">{{ $account->account_number }}</option>
                                        @endif
                                    @endforeach
                                </select>
                            </div>
                    ...
                    <table class="table-auto w-full text-left mt-5">
                        ...
                        <tr class="bg-gray-800 text-white">
                            ...
                            <th class="px-4 py-2">Bill symbol</th>
                            <th class="px-4 py-2">Bill amount</th>
                        ...
                        @foreach($results as $result)
                            @if($result->from_account===$filterAccount)
                                <tr class="bg-red-300">
                                    ...
                                    <td class="border px-4 py-2">{{$result->currency_symbol}}</td>
                                    <td class="border px-4 py-2">{{number_format($result->bill/100,2)}}</td>
